  ...
  <div class="db-layout">
    ...
    <aside class="sidebar" :class="{ 'sidebar--open': sidebarOpen }">
      ...
      <nav class="sidebar-nav">
        ...
        <router-link to="/chat"           class="nav-item nav-item--chat" active-class="nav-item--active">
          <span>Chat</span>
          <span class="nav-chat-tag">marTex</span>
        </router-link>
      </nav>
...
export default {
  ...
  data() {
    return {
      ...
      sidebarOpen: false,
      dropdownOpen: false,
      theme: getCookie('mv_theme') || 'light',
    }
  },
  computed: {
    userInitials() {
  ...
  methods: {
    ...
    toggleTheme() {
      ...
      // Cookie remembers the preferred theme across refresh and browser restart
      setCookie('mv_theme', this.theme, 365)
    },
    async logout() {
      try {
...
.nav-item--chat  { display: flex; align-items: center; justify-content: space-between; }
.nav-chat-tag {
  font-size: 0.62rem;
  font-weight: 700;
...
.theme-toggle:hover { border-color: #0f7a55; }
.hamburger {
  display: none;
  flex-direction: column;
  gap: 5px;
