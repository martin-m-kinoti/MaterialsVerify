<template>
  <app-layout title="Overview">
    ...
    <!-- Session info -->
    <div class="session-card" v-if="sessionId">
      <div class="session-item">
        <span class="session-label">Session ID</span>
        <span class="session-value session-id">{{ sessionId }}</span>
      </div>
      <div class="session-item">
        <span class="session-label">Login Time</span>
        <span class="session-value">{{ loginTimeDisplay }}</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async fetchSession() {
      try {
        const { data } = await axios.get(API + '/api/auth/session', { withCredentials: true })
...
.session-id { font-family: 'DM Sans', monospace; word-break: break-all; }
